...
-- MODELS_YML_CHECKSUM = 'ac6af781375d310505a78032b6e4a8fe'
...
comment on column user_t.public_key is 'Public key of the user to sign data. base64 encoded.';
comment on column user_t.public_key_certificate is 'Certificate for the public key of the user.';
...
CREATE TABLE poll_bulletin_board_entry_t (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    poll_id integer,
    message_type varchar(256) NOT NULL,
    message text,
    timestamp timestamptz NOT NULL,
    hash_over_last varchar(256)
);



comment on column poll_bulletin_board_entry_t.hash_over_last is 'Hash over all fields of the last entry in the same poll. Null for the first entry. Encoded as base64.';
...
CREATE TABLE nm_meeting_pwdmui_meeting_user_t (
    meeting_id integer NOT NULL REFERENCES meeting_t (id) ON DELETE CASCADE INITIALLY DEFERRED,
    meeting_user_id integer NOT NULL REFERENCES meeting_user_t (id) ON DELETE CASCADE INITIALLY DEFERRED,
    PRIMARY KEY (meeting_id, meeting_user_id)
);
...
(select array_agg(n.meeting_id ORDER BY n.meeting_id) from nm_meeting_pwdmui_meeting_user_t n where n.meeting_user_id = m.id) as default_poll_worker_in_meeting_ids,
...
(select array_agg(n.meeting_user_id ORDER BY n.meeting_user_id) from nm_meeting_pwdmui_meeting_user_t n where n.meeting_id = m.id) as poll_worker_default_meeting_user_ids,
...
(select array_agg(pb.id ORDER BY pb.id) from poll_bulletin_board_entry_t pb where pb.poll_id = p.id) as bulletin_board_entry_ids,
(select array_agg(m.id ORDER BY m.id) from meeting_user_t m where m.poll_worker_poll_ids = p.id) as poll_worker_meeting_user_ids,
...
comment on column "poll".poll_worker_meeting_user_ids is 'IDs of the meeting users, that are the poll worker for this poll.';
...
CREATE VIEW "poll_bulletin_board_entry" AS SELECT * FROM poll_bulletin_board_entry_t p;
...
ALTER TABLE meeting_user_t ADD FOREIGN KEY(poll_worker_poll_ids) REFERENCES poll_t(id) INITIALLY DEFERRED;
...
ALTER TABLE poll_bulletin_board_entry_t ADD FOREIGN KEY(poll_id) REFERENCES poll_t(id) INITIALLY DEFERRED;
...
CREATE TRIGGER tr_log_nm_meeting_pwdmui_meeting_user_t AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_pwdmui_meeting_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('meeting_user','meeting_user_id','meeting','meeting_id');
CREATE CONSTRAINT TRIGGER notify_transaction_end AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_pwdmui_meeting_user_t
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION notify_transaction_end();
CREATE TRIGGER tr_log_meeting_user_t_poll_worker_poll_ids AFTER INSERT OR UPDATE OF poll_worker_poll_ids OR DELETE ON meeting_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('poll', 'poll_worker_poll_ids');
...
CREATE TRIGGER tr_log_nm_meeting_pwdmui_meeting_user_t AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_pwdmui_meeting_user_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('meeting','meeting_id','meeting_user','meeting_user_id');
CREATE CONSTRAINT TRIGGER notify_transaction_end AFTER INSERT OR UPDATE OR DELETE ON nm_meeting_pwdmui_meeting_user_t
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION notify_transaction_end();
...
CREATE TRIGGER tr_log_poll_bulletin_board_entry AFTER INSERT OR UPDATE OR DELETE ON poll_bulletin_board_entry_t
FOR EACH ROW EXECUTE FUNCTION log_modified_models('poll_bulletin_board_entry');
CREATE CONSTRAINT TRIGGER notify_transaction_end AFTER INSERT OR UPDATE OR DELETE ON poll_bulletin_board_entry_t
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION notify_transaction_end();

CREATE TRIGGER tr_log_poll_bulletin_board_entry_t_poll_id AFTER INSERT OR UPDATE OF poll_id OR DELETE ON poll_bulletin_board_entry_t
FOR EACH ROW EXECUTE FUNCTION log_modified_related_models('poll', 'poll_id');
...
SQL nr:nr => meeting_user/default_poll_worker_in_meeting_ids:-> meeting/poll_worker_default_meeting_user_ids
FIELD 1r:nr => meeting_user/poll_worker_poll_ids:-> poll/poll_worker_meeting_user_ids
...
SQL nr:nr => meeting/poll_worker_default_meeting_user_ids:-> meeting_user/default_poll_worker_in_meeting_ids
...
SQL nr:1r => poll/bulletin_board_entry_ids:-> poll_bulletin_board_entry/poll_id
SQL nr:1r => poll/poll_worker_meeting_user_ids:-> meeting_user/poll_worker_poll_ids
...
FIELD 1r:nr => poll_bulletin_board_entry/poll_id:-> poll/bulletin_board_entry_ids
